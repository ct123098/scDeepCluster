...
class SCDeepCluster(object):
    ...
    def fit(self, x_counts, sf, y, raw_counts, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140,
        ...
        for ite in range(int(maxiter)):
            ...
            if (index + 1) * batch_size > x_counts.shape[0]:
                ...
                print("x_counts : ", x_counts.shape, x_counts.type)
                print("sf : ", sf.shape, sf.type)
                print("p : ", p.shape, p.type)
                print("raw_counts : ", raw_counts.shape, raw_counts.type)
    ...
    def output_np(self, x_count, sf, file_name):
        ...
        np.save("{}-h".format(file_name), h.astype(np.float32))
        np.save("{}-y".format(file_name), y_pred.astype(np.int32))
        print("X shape: ", x_count.shape)
        print("feature shape: ", h.shape, y_pred.shape)
        print("[INFO] write result to {}".format(file_name))
